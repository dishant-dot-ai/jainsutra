...
/* Red: Wrong View */
.wrong-view-banner { display: flex; flex-direction: column; gap: 6px; margin-bottom: 1.5rem; padding: 1rem 1.25rem 1rem 1.25rem; background: rgba(186,26,26,0.04); border-radius: 6px; }
...
# Matches any single banner CSS rule block (e.g. .principle-banner { ... })
BANNER_RULE_RE = re.compile(
    r'\.(principle|caution|wrong-view)-(banner|header|badge|name|def)\s*\{[^}]*\}\s*',
    re.DOTALL
)
# Banner section comments
BANNER_COMMENT_RE = re.compile(
    r'/\*\s*(Green: Key Jain Principle|Blue: Caution \/ Warning|Red: Wrong View)\s*\*/\s*'
)


def fix_css(css):
    # Fix simple-explanation b color
    css = re.sub(
    ...
    # Remove all existing banner CSS rules and comments
    css = BANNER_COMMENT_RE.sub('', css)
    css = BANNER_RULE_RE.sub('', css)

    # Append new banner CSS before end of style block
    css = css.rstrip() + '\n' + NEW_BANNER_CSS + '\n'
    ...
    for card in soup.find_all(class_='sutra-card'):
        ...
        for child in list(content_div.children):
            ...
            if child is translation:
                break
            if BANNER_CLASSES.intersection(child.get('class', [])):
...
def main():
    ...
    for ch in chapters:
        filepath = f'{BASE}/tattvartha-ch{ch}-v4.html'
